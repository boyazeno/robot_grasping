...
if __name__ == "__main__":
    ...
    # object_init_poses = []
    object_init_poses = [
        NameWithPose(name="box", pose=Pose(position=[0.8, 0.0, 0.0]))
    ]
    env.load(
    ...
    logging.info(
        f"Get rgb: {s.visual_data['global_camera']['rgb'].shape}, {s.visual_data['global_camera']['rgb'].max()}, {s.visual_data['global_camera']['rgb'].min()}"
    )
    logging.info(
        f"Get depth: {s.visual_data['global_camera']['depth'].shape}, {s.visual_data['global_camera']['depth'].max()}, {s.visual_data['global_camera']['depth'].min()}"
    )
    while True:
